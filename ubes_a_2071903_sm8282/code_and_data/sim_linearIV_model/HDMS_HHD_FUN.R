#################################################
###### useful functions for the PEL method ###### 
#################################################
...
#for real data
biacorr.1 <- function(B, auxi, para, supp.B)
{
  lambda <- para$lambda
  lambda <- matrix(lambda, ncol = 1)
...
# for simulation
biacorr <- function(B, auxi, para, supp.B, dw2)
{
  lambda <- para$lambda
  lambda <- matrix(lambda, ncol = 1)
  supp.lambda <- para$supp.lambda
...
# for simulation
main.iter <- function(tau, nu, init.B, y, x, z, w, dw2) #iter.max
{
  x <- as.matrix(x)
  y <- as.matrix(y)
  z <- as.matrix(z)
...
#for real data
main.iter.1 <- function(tau, nu, init.B, y, x, z, w) #iter.max
{
  x <- as.matrix(x)
  y <- as.matrix(y)
  z <- as.matrix(z)

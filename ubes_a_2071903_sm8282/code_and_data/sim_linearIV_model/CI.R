############################################################################
### use the results of HDMS_HHD_CI to compute coverage and ci length    ###
###              change the working directory when used                 ###
############################################################################
rep.num<-500 #500
beta <- matrix(c(0.5, 0.5, 0.5, 0.5), ncol=1)

# number of samples and IVs
setwd("C:/Users/Administrator/Desktop/HDMS/Rcode and sim res/sim_res_linear_bic/0.5_0.7")
n <- 100
dw <- 120
dw2 <- 8
file <- paste("Chi_ci_n=", n, "dw=", dw, "s=", dw2, ".csv", sep="")
...
write.csv(res, file = paste("ci_","n=",n,"dw=",dw, "s=", dw2, ".csv", sep= ""))
...
boxplot(cbind(chi.ci.len,normal.ci.len,tsls.ci.len)[index,],
        names = c("", "PPEL", "","", "PEL", "",
                  "", "2SLS", ""),
        col = c("white","white","white","white","white","white","white","white","white"))
